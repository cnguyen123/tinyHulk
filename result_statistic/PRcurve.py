...
def grouthtruth_json_transform(GROUNDTRUTH_JSON_FILE):
    ...
    for i in range(len(annotations)):
        ...
        # coco return [xmin, ymin, width, height] --> transform to [xmin, ymin, xmax, ymax]
        bbox = [temp[0], temp[1],temp[0] + temp[2], temp[1] + temp[3]]
        if image_id in ground_truth_dict.keys():
            print("Exist key!!")
            ground_truth_dict.get(image_id).append(bbox)
        else:
            ground_truth_dict[image_id] = [bbox]
    return  ground_truth_dict



def prediction_json_transform(PREDICTION_JSON_FILE):
    prediction_dict = {}
    ...
    for i in range(len(pd_boxes)):
        ...
        if image_id in prediction_dict.keys():
            #print("Exist key!")
            prediction_dict.get(image_id).get('boxes').append(bbox)
            prediction_dict.get(image_id).get('scores').append(score)
        else:
            bbox_list = [bbox]
            scores_list = [score]
            prediction_dict[image_id] = {'boxes': bbox_list, 'scores': scores_list}
    return prediction_dict

if __name__ == "__main__":

    print("Transforming groundtrue.json....")
    gt_boxes = grouthtruth_json_transform(GROUNDTRUTH_JSON_FILE)

    print("Transforming prediction.json...")
    pred_boxes = prediction_json_transform(PREDICTION_JSON_FILE)


    # Runs it for one IoU threshold
    iou_thr = 0.75
    print("Calculating with IOU threshold = {}...".format(iou_thr))

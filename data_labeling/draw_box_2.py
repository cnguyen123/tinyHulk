...
def draw_bounding_box_new(image_path, padding=30):
    ...
    # Define the lower and upper green color range in HSV
    lower_green = np.array([45, 40, 40])
    upper_green = np.array([100, 255, 255])

    # Create a binary mask for the green pixels
    ...
    # Invert the mask (to extract the foreground)
    foreground_mask = cv2.bitwise_not(green_mask)

    # Find contours of the foreground objects
    contours, _ = cv2.findContours(foreground_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    x_draw, y_draw, w_draw, h_draw = cv2.boundingRect(contours[0])

    x_max_draw = x_draw + w_draw
    y_max_draw = y_draw + h_draw
    ...
    cv2.rectangle(image, (x_draw, y_draw), (x_max_draw, y_max_draw), (0, 255, 0), 2)

    # Display the image with bounding boxes
    #cv2.imshow("Image with Bounding Boxes", image)
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
    #cv2.imwrite(os.path.join('../data/output_new_method_with_blur', img_name), image)
    xmi = min(max(x_draw - padding, 0), dimension_img[1])
    ymi = min(max(y_draw - padding, 0), dimension_img[0])
    xma = max(min(x_max_draw + padding, dimension_img[1]), 0)
    yma = max(min(y_max_draw + padding, dimension_img[0]), 0)
    return image, dimension_img[0], dimension_img[1], xmi, ymi, xma, yma
...
def main():
    ...
    for obj in steps:
        ...
        for f in obj_img_files:
            ...
            result, height, width, xmin, ymin, xmax, ymax = draw_bounding_box_new(obj_img_file)
            #
